		...
		(repeat
			...
			(cond 
				(
					(and
						(== local1 changeDirI)
						(SRChangeDirectory gSaveDirPtr)
					)
					(= local2 (GetSaveFiles (gGame name?) @temp4 @temp365))
					(if (== local2 -1) (= temp2 -1) (break))
					(= temp0 SRDialogType)
					(= SRDialogType (IsSaveDialog))
					(cond 
						((== SRDialogType 0))
						...
						(else (self dispose: init: strDescription @temp4 @temp365))
					)
					(selectorI draw:)
				)
...
(procedure (SRChangeDirectory strDescription &tmp temp0 [temp1 30] [temp34 30])
	(repeat
		(= temp0
			(Print

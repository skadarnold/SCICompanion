...
(procedure (drawWindow top left bottom right backgroundColor topBordColor lftBordColor botBordColor rgtBordColor bevelWid shadowWidth priority screens &tmp oldPort bevelWidCounter)
	(= oldPort (GetPort))
	(SetPort 0)
	(Graph
		grFILL_BOX
	...
	(while (< bevelWidCounter bevelWid)
		(Graph
			grDRAW_LINE
			(+ top bevelWidCounter)
			(+ left bevelWidCounter)
			(- bottom (+ bevelWidCounter 1))
			(+ left bevelWidCounter)
			lftBordColor
			priority
			-1
...
(class InsetWindow of BorderWindow
	...
	(method (open &tmp screens oldPort theTop theLeft theBottom theRight)
		(= screens VISUAL)
		(if (!= priority -1) (= screens (| screens PRIORITY)))
		(= theTop top)
		(= theLeft left)

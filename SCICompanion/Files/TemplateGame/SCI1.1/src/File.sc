...
(class File of Object
	...
	(method (readString buffer maxSize)
		(if (!= argc 2) (return 0))
		(if (not handle) (self open: fOPENFAIL))
		(return
			(if handle
	...
	;
	(method (writeString buffer &tmp temp0)
		(if (not handle) (self open:))
		(if handle
			(= temp0 0)
			(while (< temp0 argc)
				(if
				(not (FileIO fiWRITE_STRING handle [buffer temp0]))
					(return 0)
				)
				(++ temp0)
			)
	...
	;	
	(method (write buffer count &tmp temp0)
		(if (not handle) (self open:))
		(return (if handle (FileIO fiWRITE handle buffer count) else 0))
	)
	...
	(method (read buffer size)
		(if (!= argc 2) (return 0))
		(if (not handle) (self open: fOPENFAIL))
		(return (if handle (FileIO fiREAD handle buffer size) else 0))
	)
	...
	;	
	(method (seek offset whence &tmp temp0)
		(= temp0 (if (>= argc 2) whence else 0))
		(if (not handle) (self open: fOPENFAIL))
		(return (if handle (FileIO fiSEEK handle offset temp0) else 0))
	)

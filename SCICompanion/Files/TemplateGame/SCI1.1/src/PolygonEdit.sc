...
(class _EditablePolygon of List
	...
	(method (undo param1 &tmp temp0 temp1 temp2 temp3)
		...
		(while (< temp2 temp1)
			(= param1 (+ param1 2))
			(self
				add: (Memory memPEEK param1) (Memory memPEEK param1) 0
			)
			(++ temp2)
			(= param1 (+ param1 2))
		)
	...
	(method (writeFile theFile param2 &tmp temp0 temp1 temp2 temp3 [temp4 10] temp14)
		(if (== param2 srcList)
			(theFile
				writeString:
					{\t\t\t((Polygon new:)\n\n}
			...
			(while temp3
				...
				(if (not temp14) (theFile writeString: {_}))
				(theFile writeString: @temp4)
				(= temp14 0)
...
(class PolyEdit of List
	...
	(method (init)
		(DrawPic (gRoom curPic?) 100 dpCLEAR)
		(if (!= gPicNumber -1)
	...
	(method (saveForUndo param1 &tmp tempNode thePrevNode)
	...
	(method (writeObstacles)
		(if (gRoom obstacles?)
			((gRoom obstacles?) eachElementDo: #dispose release:)
		)
	...
	(method (exit &tmp [temp0 100] temp100 newFile openMode)
		(if (== state 0) (self finishAdding:))
		(self showMap: 1)
		...
		(= openMode
			(if (== temp100 1) fCREATE else fOPENCREATE) ; Replace
		)                   ; Append
		(= newFile (File new:))
		(if (not (newFile name: @local2 open: openMode))
